...
function AffiliateLoginForm() {
  ...
  useEffect(() => {
    supabase.auth.getUser().then(({ data: { user } }) => {
      if (user) router.replace("/affiliate");
    });
  }, [supabase, router]);
  ...
  async function handleGoogleSignIn() {
    const { error } = await supabase.auth.signInWithOAuth({
      provider: "google",
      options: {
        redirectTo: `${window.location.origin}/auth/callback?next=${encodeURIComponent("/affiliate")}`,
      },
    });
    ...
    <div className="flex min-h-screen items-center justify-center bg-zinc-50 dark:bg-zinc-950">
      <div className="w-full max-w-sm space-y-8 px-4">
        ...
        <div className="space-y-4">
          ...
          <div className="relative">
            ...
            <div className="relative flex justify-center text-xs">
              <span className="bg-zinc-50 px-2 text-zinc-400 dark:bg-zinc-950">or continue with email</span>
            </div>
          </div>
        </div>
        ...
        <p className="text-center text-sm text-zinc-500 dark:text-zinc-400">
          New affiliate?{" "}
          <Link href="/affiliate/register" className="font-medium text-emerald-600 hover:underline">Apply here</Link>
        </p>
      </div>
    </div>
  );
