-- Fix: selecting the "Unknown" bucket (Seniority / Function / Industry / Size / Country)
-- in My Events returned zero contacts.
--
-- The breakdown sidebar labels NULL-axis rows as the literal key 'Unknown' (coalesce in
-- the facet builder), and event_filtered_facts (the preview/count path) was already patched
-- on 2026-06-22 to match 'Unknown' against NULL rows. But the contact-list RPC
-- get_subscribed_event_contacts still did a plain `col = any(selected)` match, and
-- `NULL = 'Unknown'` is never true -- so picking "Unknown" matched nothing even though the
-- sidebar showed a count. This adds the same explicit NULL branch on each coalesced axis.
-- Role is excluded: it is never NULL.

CREATE OR REPLACE FUNCTION public.get_subscribed_event_contacts(p_event_id uuid, p_filter text DEFAULT 'all'::text, p_limit integer DEFAULT NULL::integer, p_offset integer DEFAULT 0, p_filters jsonb DEFAULT '{}'::jsonb)
