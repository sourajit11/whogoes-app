-- UX fix: collapse the two seniority catch-alls into one chip.
--
-- Unlike industry (where 'Other / Unknown' and NULL both just meant "we don't know"),
-- seniority has a genuine internal distinction:
--   * seniority_bucket = 'Other'  — the contact WAS classified, but the title fits none
--     of the standard ladder tiers (consultant, student, advisor, retired, ambiguous)
--   * seniority_bucket IS NULL     — never classified (no title, or classifier not run)
-- That distinction matters for pipeline health but is meaningless to a customer
-- filtering an attendee list — both read as "not one of the named tiers", so showing
-- two adjacent chips ("Other" + "Unknown") recreates the confusion we just fixed for
-- industry. Merge them into a single 'Other / Unknown' chip, matching the industry axis.
-- The raw seniority_bucket = 'Other' vs NULL split is untouched (still available for audits).
--
-- Mirrors 20260705150000 (industry merge) across the same 4 functions:
--   1. get_event_filter_facets   — by_seniority maps NULL + 'Other' -> 'Other / Unknown'
--   2. event_filtered_contact_ids / event_filtered_facts — selecting 'Other / Unknown'
--      (or legacy 'Unknown') matches both NULL and 'Other' rows; legacy 'Other' still works
--   3. get_event_filter_preview  — SAMPLE + rows show the merged seniority label

-- 1. Facet builder: merge NULL and 'Other' into a single seniority key.
...
-- 2a. Canonical filter helper: 'Other / Unknown' (or legacy 'Unknown') matches NULL + 'Other'.
...
-- 2b. Denormalized sibling (unlocked My Events view): same seniority match.
...
AS $function$
  ...
  WHERE f.event_id = p_event_id
    AND (not (p_filters ? 'seniority') or f.seniority = any(array(select jsonb_array_elements_text(p_filters->'seniority')))
         or ((f.seniority is null or f.seniority = 'Other') and ((p_filters->'seniority') ? 'Unknown' or (p_filters->'seniority') ? 'Other / Unknown')))
...
-- 3. Preview: SAMPLE + rows show the merged seniority label.
  ...
  select json_build_object(
    ...
    'sample', (
      select case when s.contact_id is null then null else json_build_object(
        ...
        'country', c.country,
        'seniority', case when c.seniority_bucket is null or c.seniority_bucket = 'Other' then 'Other / Unknown' else c.seniority_bucket end,
    ...
    'rows', (
      select coalesce(json_agg(json_build_object(
        'current_title', c.current_title,
        'seniority', case when x.seniority is null or x.seniority = 'Other' then 'Other / Unknown' else x.seniority end,
        'function', x.func, 'industry', coalesce(x.industry, 'Other / Unknown'), 'size', x.sizeb,
