-- Refine: ONLY bare reposts are Expected attendees. Mentions stay Confirmed attendees
-- (the existing pipeline already treats mentioned contacts as attendees). post_author and
-- speakers remain Confirmed. Sponsor/Exhibitor/Organizer (company-level) still win.
...
AS $function$
  ...
  from (
    select distinct on (ce.contact_id)
      ...
      -- effective per-contact role. distinct-on ordering prefers a confirming row
      -- (speaker, then post_author/mentioned) so a contact only falls to expected when
      -- their ONLY evidence is a bare repost.
      case
