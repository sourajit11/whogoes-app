-- My Events: return + filter on the effective per-contact role, so "Expected attendee"
-- (repost/mention-only) is distinct from "Attendee" (first-person/speaker). Same logic as
-- event_filtered_contact_ids. Return signature unchanged.
...
RETURNS TABLE(
  ...
  event_role text, company_size_bucket text, company_industry_bucket text, is_speaker boolean
)
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
    ...
    WHERE cca.event_id = p_event_id
      ...
      AND (not (p_filters ? 'role')      or (
            case
              ...
              when exists(select 1 from contact_events cep where cep.contact_id = cca.contact_id and cep.event_id = p_event_id and (cep.is_speaker = true or cep.source_type = 'post_author')) then 'attendee'
  ...
  SELECT
    ...
    case
      when coalesce(cer.role,'attendee') in ('organizer','sponsor','exhibitor') then cer.role
      when exists(select 1 from contact_events cep2 where cep2.contact_id = c.id and cep2.event_id = p_event_id and (cep2.is_speaker = true or cep2.source_type = 'post_author')) then 'attendee'
      else 'expected_attendee'
    end AS event_role,
