-- Improve queue ordering: process upcoming first, then companies with the biggest presence
-- at the event (contact count as a proxy for a real company likely to have US SDRs), so we
-- skip alphabetical noise ("&commit", ":probabl.") and hit real companies early.
