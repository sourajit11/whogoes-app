...
AS $$
DECLARE
  v_cron jsonb; v_queue jsonb; v_stale jsonb; v_facts jsonb;
  v_dist jsonb; v_tiers jsonb; v_hygiene jsonb; v_consistency jsonb;
  v_sample_events int := 0; v_sample_drift int := 0;
  v_examples jsonb := '[]'::jsonb; r record; v_ndiff int;
BEGIN
  ...
  SELECT jsonb_build_object('facts_behind_roles', count(*)) INTO v_facts
  ...
  LOOP
    ...
    END IF;
  END LOOP;
  v_consistency := jsonb_build_object('events_sampled', v_sample_events,
    'events_with_role_drift', v_sample_drift, 'examples', v_examples);
  ...
  RETURN jsonb_build_object(
    ...
    'facts', v_facts, 'consistency', v_consistency,
