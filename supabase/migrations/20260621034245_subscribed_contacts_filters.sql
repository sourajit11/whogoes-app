-- Phase 5 (cont): apply the same ICP filters to the My Events display. Filters narrow the
-- already-unlocked set. Same jsonb contract as the facets/unlock helpers.
drop function if exists public.get_subscribed_event_contacts(uuid, text, integer, integer);
...
BEGIN
  ...
  SELECT
    ...
    cem.email, cem.status AS email_status, cem.provider AS email_provider,
