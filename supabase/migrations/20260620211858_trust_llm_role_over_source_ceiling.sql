-- When the LLM has read a post's intent (posts.extracted_event_role set), trust it: drop the
-- source-class ceiling, which was a crude guardrail for the deterministic regex and wrongly
-- downgraded genuine company-page sponsor declarations tagged source_type='repost' (a page
-- resharing its own announcement). The LLM already labels bare reshares as attendee. Confidence
-- now comes from the LLM's own role_confidence (high/medium -> confirmed, low -> likely); the
-- non-LLM attendee baseline keeps its source/post_type-based confidence.
...
as $$
  ...
  ranked as (
    select c.*,
      -- LLM role (intent) when present, else deterministic attendee baseline. No ceiling clamp.
      coalesce(
