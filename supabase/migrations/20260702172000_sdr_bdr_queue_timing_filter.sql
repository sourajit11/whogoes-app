-- Allow pulling the queue for a single timing segment (past | upcoming), so a run can be
-- targeted (e.g. 10 past + 10 upcoming) instead of upcoming-first only.
drop function if exists public.sdr_bdr_next_companies(int);
create or replace function public.sdr_bdr_next_companies(p_limit int default 200, p_timing text default null)
...
language sql as $$
  ...
  where c.rn = 1 and not c.is_host
    and (p_timing is null or c.timing = p_timing)
