...
alter table shootday_partners_discovery.organizers
  add column if not exists organized_count integer,
  add column if not exists sponsored_count integer,
...
drop function if exists public.spd_set_organizer_qualification(uuid,text,text,text,text,boolean,text);
...
begin
  update shootday_partners_discovery.organizers set
    ...
    upcoming_events      = p_upcoming_events,
    updated_at           = now()
  where id = p_id
